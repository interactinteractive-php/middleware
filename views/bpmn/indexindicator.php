...
<div class="bpmn-main-container">
  ...
  <script>
    var bpmnModeler, modeling, elementFactory, canvas, $tooltip, bpmnModelerSaved;

    $.cachedScript('https://unpkg.com/bpmn-js@8.9.1/dist/bpmn-modeler.production.min.js').done(function() {
      ...
      callMetaParameter = async function () {
          ...
          var response = $.ajax({
            type: "post",
            url: "mdprocessflow/checkBpIndicator",
            data: {
              domainbp: domainbp,
            },
            dataType: "json",
      ...
      callDataModelParameter = async function () {
          ...
          $.ajax({
              type: 'post',
              url: 'mdprocessflow/getInputMetaParameterIndicator',
              data: {
                mainBpId: indicatorId, 
                doProcessId: domainbp[0], 
                connection2: result.xml,
                indicatorName: '<?php echo $this->indicatorName; ?>',
              },
              beforeSend: function () {
                Core.blockUI({
                  message: "Loading...",
                  ...
                  $dialog.dialog({
                      ...
                      buttons: [
                          {text: plang.get('save_btn'), class: 'btn green-meadow btn-sm', click: function () {
                              $.ajax({
                                  type: 'post',
                                  url: 'mdprocessflow/saveMetaProcessParameterIndicator',
                                  data: $('#metaProcessParameter-form').serialize(),
                                  dataType: "json",
                                  beforeSend: function () {
                                      Core.blockUI({boxed: true, message: 'Хадгалж байна...'});
      ...
      */
      async function exportDiagram2() {
        ...
        try {
          ...
          $.ajax({
            type: "post",
            url: "mdprocessflow/saveBpmnDraftIndicator",
            data: {
              xml: result.xml,
              mainBpId: mainBpId
            },
          ...
          $.ajax({
            type: "post",
            url: "mdprocessflow/saveBpmnIndicator",
            data: {
              xml: result.xml,
              mainBpId: mainBpId
            },
      ...
      */
      async function openDiagram(bpmnXML) {
        ...
        try {
          ...
          var bpmRegistry = bpmnModeler.get('elementRegistry');        
          // var elements = bpmRegistry._elements;            
          bpmnModelerSaved = Object.keys(bpmRegistry._elements);
    ...
    window.onbeforeunload = function () {
      var bpmRegistry = bpmnModeler.get('elementRegistry'),
          bpmLen = Object.keys(bpmRegistry._elements).length;
      if(bpmLen != Object.keys(bpmnModelerSaved).length && bpmLen > 2) {
          return true;
      } else {
          return undefined;
      }
