<form class="form-horizontal" role="form" method="post" id="metaProcessParameter-form">
    <div class="row">
        <div class="col-md-12">
            <div class="form-group row mb0 d-none">
                <label class="col-md-3 col-form-label">Data Model:</label>
                <div class="col-md-9">
                    <p class="form-control-plaintext">
                        <?php
                        echo Form::hidden(array('name' => 'metaDataId', 'id' => 'metaDataId', 'value' => $this->mainIndicatorId));
            ...
            <table class="table table-sm table-hover metaProcessParameter" id="bpChildParameterInput" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        ...
                        <th class="pl30">Data model</th>
                        <th class="text-center" style="width:40px;">Is input</th>
                        <th style="width: 20%;">Data model param id</th>
                        <th style="width: 20%;">Data model param path</th>
...
<script type="text/javascript">
    $(function () {
        ...
        $('#metaProcessParameter-form').on('change', 'select[name="<?php echo $this->doProcessid; ?>inputDoneBpId[]"]', function () {
            if ($(this).find('option:selected').data('metacode') == '') {
                changeDoneBpParam(this);
            } else {
                $(this).closest('tr').find('input[name="<?php echo $this->doProcessid; ?>inputMetaDataIsProcess[]"]').val('1')
                changeDoneBpParam2(this);
            }
        });
        ...
        if (doneBpId != '') {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    criteriaData: {
                        filterMainId: [{ operator: "=", operand: doneBpId }],
                    },
                },
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(data){
                    ...
                    if (data.result) {
                        $.each(data.result, function(){
                            if (inputDoBpParamPath == this.columnnamepath) {
                                criteriaValueCombo.append($("<option selected='selected' />").val(this.columnnamepath).text(this.labelname));
                                parent.find('input[name="<?php echo $this->doProcessid; ?>inputDoneBpParamPath[]"]').val(inputMetaDataName);
                                setInputDoneBpParamPath = 1;
                            } else {
                                criteriaValueCombo.append($("<option />").val(this.columnnamepath).text(this.labelname));
                            }
                        });
    ...
    function changeDoneBpParam2(elem) {
        var _this = $(elem);
        var doneBpId = _this.val();
        var parent = _this.closest('tr');
        var isCheck = 0;
