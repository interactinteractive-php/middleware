<ul class="dv-explorer5 dv-explorer" id="main-item-container">
    <?php
    if ($this->isBack) {
    ?>
    ...
    <li class="dv-explorer-row-f" id="<?php echo $folderRow['META_VALUE_CODE']; ?>">	
        <a href="javascript:;" onclick="selectDataViewByCategory_<?php echo $this->dataViewId; ?>('<?php echo $folderRow['META_VALUE_ID']; ?>')" class="folder-link" title="<?php echo $folderRow['META_VALUE_NAME']; ?>">
            <div class="first-title">
                <?php echo $folderRow['META_VALUE_CODE']; ?>
            </div>
            <div class="dv-img-container">
                <div class="dv-img-container-sub">
                    <img class="dv-directory-img" src="assets/core/global/img/meta/folder_transparent_4.png"/>
                </div>
            </div>
            <div class="second-title">
          ...
          foreach ($this->recordList as $recordRow) {
            ...
            ?>
              <li class="dv-explorer-row"<?php eval($rowcolor); ?>>	
                <div class="selected-row-link" title="<?php echo strip_tags($recordRow[$this->name1]); ?>" data-row-data="<?php echo $rowJson; ?>" onclick="<?php eval($onClick); ?>">
                  <div class="first-title">
                      <?php eval($name2); ?>
                  </div>
                  <div class="dv-img-container">
...
$(function(){
    if (!$("link[href='middleware/assets/css/gridlayout/explorer5.css']").length) {
        $("head").prepend('<link rel="stylesheet" type="text/css" href="middleware/assets/css/gridlayout/explorer5.css"/>');
    }
});
</script>