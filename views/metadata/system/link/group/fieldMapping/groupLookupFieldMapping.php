...
<div class="table-scrollable overflowYauto" style="max-height: 500px">
    <table class="table group-lookup-field-map-tbl">
        ...
        <?php
        if ($this->tempField) {
            if (count($this->fieldsMapping['fieldMappingParamMetaId'][$this->paramPath]) == 0) {
        ...
        <?php    
            ...
            foreach ($this->fieldsMapping['fieldMappingParamMetaId'][$this->paramPath] as $k => $fieldMappingParamMetaId) {
        ?>    
            <tr>
                <td>
                    <?php 
                    echo Form::comboOneGroupChildMetas(
                        array(
                            ...
                            'required' => 'required', 
                            'style' => 'width:330px', 
                            'value' => $this->fieldsMapping['fieldMappingParamFieldPath'][$this->paramPath][$k]."-".$fieldMappingParamMetaId
                        )
                    ); 
                    ?>
                </td>
                <td>
                    ...
                    echo Form::select(
                        array(
                            ...
                            'op_text' => 'META_DATA_CODE| |-| |META_DATA_NAME', 
                            'style' => 'width:330px', 
                            'value' => $this->fieldsMapping['fieldMappingLookupFieldPath'][$this->paramPath][$k]."-".$this->fieldsMapping['fieldMappingLookupMetaId'][$this->paramPath][$k]
                        )
                    ); 
                    ?>
                </td>
