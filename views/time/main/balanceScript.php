<script type="text/javascript">
    ...
    $(function () {        
        ...
        setTimeout(function(){
            ...
            var dynamicHeightTna = $(window).height() - 70;

            $(window).scroll(function(){
                if(tnaSidebarWidth > 0) {
                    var scrollPos = offsetTopExpression - $(this).scrollTop();
                    if(scrollPos <= 0) {
                        $(".tna-sidebar-container").addClass("tnasidbar-viewer-class").css({'width':(tnaSidebarWidth + 30)+'px', 'height': dynamicHeightTna});
                    } else
                        <?php if (defined('CONFIG_TOP_MENU') && CONFIG_TOP_MENU) { ?>
                            $(".tna-sidebar-container").removeClass("tnasidbar-viewer-class").removeAttr('style').css('margin-top', '85px');
                        <?php } 
                            else { ?>
    ...
    function tnaRenderSidebar(row, index, $uniqId) {
        ...
        $.ajax({
            type: 'post',
            url: 'mdtime/getBalanceDetailListV2',
                        ...
                        sideBarHtml += '<div class="card-body">';
                            sideBarHtml += '<div class="clearfix w-100">';
                                ...
                                sideBarHtml += '<div class="clear">';
                                    ...
                                    sideBarHtml += '</div>';
                                sideBarHtml += '</div>';
                                sideBarHtml += '<span class="float-right" style="margin-right: -7px;"><?php echo $this->balanceBtn['item'];?></span>';
                            sideBarHtml += '</div>';
                        sideBarHtml += '</div>';
                    sideBarHtml += '</div>';
                    var _timeInit = 'timeInit';
                    sideBarHtml += '<div class="panel panel-default bg-inverse grid-row-content" style="margin-top: -21px; height: 600px; overflow-y: auto; overflow-x: hidden;">';
                        sideBarHtml += '<table class="table sheetTable">';
                            sideBarHtml += '<tbody>';
                                sideBarHtml += '<tr class="intime">';
                                    ...
                                    sideBarHtml += '<td style="width: 80px">';
                                        sideBarHtml += '<input class="intime '+ _timeInit +' " ' + (!isStartTimeEdit ? 'readonly style="background-color: #EBEBE4" ' : '') + 'name="change_intime['+selectedRowUniqueId+']" placeholder="hh:mm" onchange="changeListenerIO(this);" type="text" value="'+ row.IN_TIME +'">';
                                    ...
                                    sideBarHtml += '<td>'
                                                        ...
                                                        + '<button type="button" id="" class="btn btn-sm employeeBalanceDescriptionTime float-right" title="Тайлбар"><i class="fa fa-font"></i> </button>'
                                                    + '</td>';
                                sideBarHtml += '</tr>';
                                ...
                                sideBarHtml += '<tr class="outtime">';
                                    ...
                                    sideBarHtml += '<td>';
                                        sideBarHtml += '<input class="outtime '+ _timeInit +' " ' + (!isEndTimeEdit ? 'readonly style="background-color: #EBEBE4" ' : '') + 'name="change_outtime['+selectedRowUniqueId+']" placeholder="hh:mm" type="text" onchange="changeListenerIO(this);" value="'+ row.OUT_TIME +'">';
                                ...
                                for (var i = 0; i < detail.length; i++) {
                                    ...
                                    _btn += '<button type="button" '+ disabled +' id=' + i + ' class="btn btn-sm employeeBalanceDescription ml0 mr0" title="Тайлбар"><i class="fa fa-font"></i> </button>';
                                ...
                                if (!golomtView) {
                                    sideBarHtml += '<tr>';
                                        sideBarHtml += '<td style="background-color:#95A5A6 !important; width: 200px; color:#FFF !important;" class="left-padding">Дэлгэрэнгүй</td>';
                                        sideBarHtml += '<td style="background-color:#95A5A6 !important; padding-right:2px;"></td>';
                                        sideBarHtml += '<td>';
                                            sideBarHtml += '<div class="btn-group">';
                                                sideBarHtml += '<button style="border-radius:0 !important;" type="button" class="btn btn-sm grey-cascade employeeBalanceDetail ml0 mr0" title="Дэлгэрэнгүй"><i class="fa fa-list-alt"></i> </button>';
                                                sideBarHtml += '<button type="button" class="btn btn-sm blue-madison ml0 mr0 isLock" title="Түгжих"><i class="fa fa-lock"></i> / <i class="fa fa-unlock-alt"></i></button>';
                                            sideBarHtml += '</div>';
                                        sideBarHtml += '</td>';
                                    sideBarHtml += '</tr>';
                                }
                                ...
                                sideBarHtml += '<tr>';
                                    sideBarHtml += '<td style="width: 200px; color:#FFF !important;" class="left-padding"></td>';
                                ...
                                sideBarHtml += '<tr id ="EMPLOYEE_DESCRIPTION_' + row.EMPLOYEE_KEY_ID + '_' + row.CH_BALANCE_DATE + '" class="hidden">';
                                    sideBarHtml += '<td style="background-color:#f36a5a !important; width: 200px; color:#FFF !important;" class="left-padding">CASE Дэлгэрэнгүй</td>';
