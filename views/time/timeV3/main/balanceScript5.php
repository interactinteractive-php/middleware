<script type="text/javascript">
    ...
    var tmsParentPageSize = <?php echo Config::getFromCache('tmsPageNumber') ? Config::getFromCache('tmsPageNumber') : '30'; ?>;
    ...
    $(function () {
        if (tmsDefaultFilter == '1') {
            setTimeout(function(){
                $(".search-tms-btn", "#tnaTimeBalanceWindow<?php echo $this->uniqId ?>").click();
            }, 200);
        }
        $("#tnaTimeBalanceWindow<?php echo $this->uniqId ?>").on('change', "#newDepartmentId_valueField", function(){
            $(".search-tms-btn", "#tnaTimeBalanceWindow<?php echo $this->uniqId ?>").click();
        });
        ...
        $.getScript("assets/custom/addon/plugins/jquery-multiselect/js/jquery.multiselect.js" ).done(function( script, textStatus ) {
            $.getScript("assets/custom/addon/plugins/jquery-multiselect/js/jquery.multiselect.filter.js").done(function( script, textStatus ) {
                ...
                $('#ui-multiselect-positionId_<?php echo $this->uniqId ?>-option-0').parent().parent().remove();                  
                                ...
                                for (var i = 0; i < detail.length; i++) {
                                    ...
                                    sideBarHtml += '<tr class="causeClassName_' + detail[i].CAUSE_TYPE_ID + '">';
                                        ...
                                        sideBarHtml += '<td>';
                                            ...
                                            sideBarHtml += '<input '+ disabled +' class="cause_type_value_display '+ _timeInit +' "'+ctypeDisable+' id="cause_type_code_'+detail[i].CODE+'" data-name="cause_type_value_display"'+readOnly+' name="cause_type_value_display['+selectedRowUniqueId+']['+ detail[i].CAUSE_TYPE_ID +']" placeholder="hh:mm" type="text" value="' + minutToTime(detail[i].V_TIME) + '" onchange="setMinut(this);">';
                                        sideBarHtml += '</td>';
