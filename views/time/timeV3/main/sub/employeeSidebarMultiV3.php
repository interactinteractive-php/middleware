<form id="multiChangeBalanceForm">
    <div class="panel panel-default bg-inverse grid-row-content">
        <table class="table sheetTable">
            <tbody>
                <tr>
                    <td style="width: 150px;" class="left-padding"><?php echo Form::label(array('text'=>'ИРСЭН ЦАГ', 'for'=>'Ирсэн цаг'));?></td>
                    <td style="width: 50px;" colspan="<?php echo Config::getFromCache('CONFIG_TNA_HISHIGARVIN') ? '2' : '' ?>">
                        <input class="inTime timeInit <?php echo (!$this->isStartTimeEdit) ? 'readonly-inTime' : '' ?>" onchange="changeListenerIO(this);" data-name="inTime" name="inTime" placeholder="hh:mm" <?php echo (!$this->isStartTimeEdit) ? 'readonly="readonly" style="background-color: #EBEBE4"' : '' ?> type="text" value="<?php echo isset($this->params[0]['IN_TIME']) ? $this->params[0]['IN_TIME'] : '' ?>">
                        <input name="detect_change_intime" placeholder="hh:mm" disabled type="hidden" value="">
                    </td>
                ...
                <tr>
                    <td style="width: 150px;"class="left-padding"><?php echo Form::label(array('text'=>'ЯВСАН ЦАГ', 'for'=>'Явсан цаг'));?></td>
                    <td style="width: 50px;" colspan="<?php echo Config::getFromCache('CONFIG_TNA_HISHIGARVIN') ? '2' : '' ?>">
                        <input class="outTime timeInit <?php echo (!$this->isEndTimeEdit) ? 'readonly-inTime' : '' ?>" data-name="outTime" onchange="changeListenerIO(this);" name="outTime" placeholder="hh:mm" <?php echo (!$this->isEndTimeEdit) ? 'readonly="readonly" style="background-color: #EBEBE4"' : '' ?> type="text" value="<?php echo isset($this->params[0]['OUT_TIME']) ? $this->params[0]['OUT_TIME'] : '' ?>">
                        <input name="detect_change_outtime" placeholder="hh:mm" disabled type="hidden" value="">
                    </td>
            ...
            <tbody>
                <?php 
                    ...
                    foreach ($this->tnaCauseType as $k=>$row) {
                        ...
                        $btnDesc .= ' title="Тайлбар"><i class="fa fa-comment"></i> </button>';
                        ...
                        $html .= '<tr>';
                            ...
                            $html .= '<td style="width: 50px;">';
                                ...
                                $html .= '<input name="cause_type_value[' . $row['CAUSE_TYPE_ID'] . ']" data-name="cause_type_value" type="hidden" value="0">';
                            ...
                            $html .= '</td>';
                            $html .= '<td style="width: 190px;"><input type="hidden" name="description[' . $row['CAUSE_TYPE_ID'] . ']" value=""><div class="btn-group float-right">' . $btn . $btnDesc . '</div></td>';
                        $html .= '</tr>';
                    }
                    echo $html;
                ?>
