    ...
    if ($this->mainBpTab['ticket'] == '0') {
        $mainProcessBtnBar .= '<div class="ml-auto">
            ...
            array(
                ...
                'onclick' => 'runBusinessProcess(this, \'' . $this->dmMetaDataId . '\', \'' . $this->uniqId . '\', ' . json_encode($this->isEditMode) . ', undefined, qrGenerateProcessAfterSave);',
...
<script type="text/javascript">
    ...
    $(function() {
        Core.blockUI({
            target: '#bp-window-<?php echo $this->methodId; ?>',
            boxed: true,
            message: 'Мэдээлэл татаж байна, Түр хүлээнэ үү...'
        });
    });
        ...
        if (typeof splitPaths[2] === 'undefined') {
            ...
            if ($tagName === 'select') {
                ...
                if (selSel.length) {
                    ...
                    if (1 < selSel.children().length) {
                        selSel.children().each(function (i2, r2) {
                            if (i2 < selSel.children().length) {
                                $(r2).remove();
                            }
                        });
                    }
                    ...
                    if (typeof _this.attr('select2-editing-notemp') !== 'undefined' && _this.attr('select2-editing-notemp') === '1') {
                        setTaxonamyFunctionNoTemp_<?php echo $this->methodId; ?>(_this, _this.closest('.detail-template-body-rows'));
                    }
                    
                    /*
                    if (thisval) {
                        console.log('--------level 1 ----------' + cindex);
                    }*/
                }
            ...
            if (typeof splitPaths[3] !== 'undefined') {
                ...
                var selectorOptionInd = secondMainSelector.find('table[data-table-path="' + splitDataPaths[0] + '.' + splitPaths[1] + '.' + splitPaths[2] + '"] > tbody:first > tr:eq(' + subKeyIndex + ')').find('td input[data-path=\'' + splitDataPaths[0] + '.' + splitPaths[1] + '.' + splitPaths[2] + '.orderNum\']');
                ...
                //console.log('--------level 3 ----------' + subKeyIndex);

                if ($tagName === 'select') {
                    setTimeout(function() {
                        if (selectorOption.length) {
                            selectorOption.prop('disabled', false);
                            selectorOption.children().val(thisval).attr('selected', 'selected');;

                        } else {
                            selectorOption2.prop('disabled', false);
                            selectorOption2.children().val(thisval).attr('selected', 'selected');;
                        }
                ...
                var selectorOptionInd = mainSelector.find('table > tbody:first > tr:eq(' + cindex + ')').find('table[data-table-path="' + splitDataPaths[0] + '.' + splitPaths[1] + '"] > tbody:first > tr:eq(' + subIndex + ') > td').find('input[data-path=\'' + splitDataPaths[0] + '.' + splitPaths[1] + '.orderNum\']');
    ...
    function multiSelectionRender_<?php echo $this->methodId ?>(lookupMetaDataId, parentId, tag, element, mainSelector) {
        ...
        if (typeof $(mainSelector).attr('data-in-param') !== 'undefined') {
            ...
            paramData.push({
                ...
                name: 'serviceBookId',
                value: $('#bp-window-render-<?php echo $this->methodId ?>').find('input[data-path="id"]').val()
            });
            ...
            element.empty().append('<div class="hidden"></div>' + $html).promise().done(function() {
                ...
                $('.list-jtree-<?php echo $this->methodId ?>').on("changed.jstree", function(e, data) {
                    ...
                    if (data.action === "select_node") {
                        ...
                        if (selectedJTreePathId_<?php echo $this->methodId ?>) {
                            $dataSelected = selectedJTreePathId_<?php echo $this->methodId ?>;
                            ...
                            selectedJTreePathIdTemp_<?php echo $this->methodId ?> = [];
                            selectedJTreePathText_<?php echo $this->methodId ?> = [];
                            $.each($dataSelected, function(key, value) {
                                if (typeof data['instance']['_model']['data'] !== 'undefined' && typeof data['instance']['_model']['data'][value] !== 'undefined') {
                                    ...
                                    $.each(data['instance']['_model']['data'], function (iindex, irow) {
                                        if (typeof irow['children'] !== 'undefined' ) {
                                            $.each(irow['children'], function (ci, cr) {
                                                if (value === cr) {
                                                    if ($.inArray(iindex, selectedJTreePathId_<?php echo $this->methodId ?>) < 0 && typeof irow['original'] !== 'undefined') {
                                                        selectedJTreePathId_<?php echo $this->methodId ?>.push(irow['original']['id']);
                                                        selectedJTreePathIdTemp_<?php echo $this->methodId ?>.push(irow['original']['id']);
                                                        selectedJTreePathText_<?php echo $this->methodId ?>.push(irow['original']['text']);
                                                    }
                                                }
                                            });
                                        }
                        ...
                        for (var el = 0; el < selectedElms.length; el++) {
                            ...
                            var $_ss = _dataPath.split('.');
                            var $dataSelected = $('table[data-table-path="'+ $_ss[0] +'"]').find('select[data-path="'+ _dataPath +'"]').attr('data-edit-value');
                            
                            if (typeof $dataSelected !== 'undefined') {
                                var $dataSelectedArr = $dataSelected.split(',');
                                
                                selectedJTreePathId_<?php echo $this->methodId ?> = [];
                                selectedJTreePathIdTemp_<?php echo $this->methodId ?> = [];
                                selectedJTreePathText_<?php echo $this->methodId ?> = [];
                                
                                $.each($dataSelectedArr, function(key, value) {
                                    if (typeof data['instance']['_model']['data'] !== 'undefined' && typeof data['instance']['_model']['data'][value] !== 'undefined') {
                                        if (typeof data['instance']['_model']['data'][value]['children'] !== 'undefined') {
                                ...
                                console.log($('table[data-table-path="'+ $_ss[0] +'"]').find('select[data-path="'+ _dataPath +'"]'));
                            }
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        bp_window_<?php echo $this->methodId; ?>.on("change", 'input, select', function() {
            var mainSelector = '',
                selectorOption = '',
                selectorOption2 = '',
                selSel = '',
        ...
        for (tloop; tloop < tlength; tloop++) {
            if (taxonamyObjParse[tloop].IS_ADD_FOLLOW === '1') {
                ...
                if (taxonamyObjParse[tloop].EXPRESSION == '' || taxonamyObjParse[tloop].EXPRESSION == null) {
                    ...
                    $mainSelectorOption.append('<p class="detail-template-body-rows">' + $(elem).parent().find('select').prop('outerHTML') + ' <a href="javascript:;" title="Устгах" style="margin-right: -20px;" onclick="templateDtlRemoveRow_<?php echo $this->methodId; ?>(this, \'' + taxonamyObjParse[0].PATH + '\', \'' + uniqId + '\')" class="btn btn-xs btn-danger float-right tax-remove"><i class="fa fa-trash"></i></a></p>');
    ...
    function templateDtlRemoveRow_<?php echo $this->methodId; ?>(elem, path, uniqId, uniqType) {
        ...
        if (uniqId != '') {
            ...
            if (uniqElement.length) {
                ...
                uniqElement.find('.widget-party-container').find('span[data-uindex="' + dataUindex + '"]').addClass('hidden');
        ...
        if ($(elem).closest('.detail-template-body-sub-rows').length) {
            ...
            if ($(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').length && widgetCode !== 'widget_none') {
                $(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').each(function() {
                    ...
                    $(this).children('.detail-template-body-rows:eq(' + tempInd + ')').children('.detail-template-body-sub-rows:eq(' + cindex + ')').addClass('hidden');
                });
            } else {
                //$(elem).closest('.detail-template-body-sub-rows').remove();
                $(elem).closest('.detail-template-body-sub-rows').addClass('hidden');
            }
            ...
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + pth[0] + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('table[data-table-path="' + path + '"] > tbody:first > tr:eq(' + cindex + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').val('removed');
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + pth[0] + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('table[data-table-path="' + path + '"] > tbody:first > tr:eq(' + cindex + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').attr('value', 'removed');
            ...
            if ($(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').length && widgetCode !== 'widget_none') {
                $(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').each(function() {
                    ...
                    $(this).children('.detail-template-body-rows:eq(' + tempInd + ')').addClass('hidden');
                });
            } else {
                //$(elem).closest('.detail-template-body-rows').remove();
                $(elem).closest('.detail-template-body-rows').addClass('hidden');
            }

            if (typeof uniqType !== 'undefined') {
                
                $mainSelector.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').val('removed');
                $mainSelector.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').attr('value', 'removed');
                //$mainSelector.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').remove();
                //bpSetRowIndex($mainSelector);
            } else {
                console.log(bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="' + path + '.rowState"]:eq(0)'));
                //bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').remove();
                bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').val('removed');
                bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="' + path + '.rowState"]:eq(0)').attr('value', 'removed');
            }
        }
    }
    ...
    function templateDtlKeyRemoveRow_<?php echo $this->methodId; ?>(elem, path, uniqId) {
        ...
        if (uniqId != '') {
            ...
            if (uniqElement.length) {
                //uniqElement.find('.widget-party-container').find('span[data-uindex="' + dataUindex + '"]').remove();
                uniqElement.find('.widget-party-container').find('span[data-uindex="' + dataUindex + '"]').addClass('d-none');
                uniqElement.find('span.party-title-counter').text('(' + (uniqElement.find('.widget-party-container').children('span').length) + ')');
            }
        } else {
            var uniqElement = $('body').find('div[data-path-code="' + $(elem).closest('.detail-template-body').attr('data-dtl-template-path') + '"]');
        ...
        if ($(elem).closest('.detail-template-body-subkey-rows').length) {
            ...
            if ($(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').length && widgetCode !== 'widget_none') {
                $(elem).closest('.bp-template-wrap').find('span[data-dtl-template-widget="' + widgetCode + '"]').each(function() {
                    ...
                    */
                    $(this).children('.detail-template-body-rows:eq(' + ptempInd + ')')
                        .children('.detail-template-body-sub-rows:eq(' + tempInd + ')')
                        .children('.detail-template-body-subkey-rows:eq(' + cindex + ')').addClass('d-none');
                    $(this).children('.detail-template-body-rows:eq(' + ptempInd + ')')
                        .children('.detail-template-body-subkey-rows:eq(' + cindex + ')').addClass('d-none');
                });
            } else {
                //$(elem).closest('.detail-template-body-subkey-rows').remove();
                $(elem).closest('.detail-template-body-subkey-rows').addClass('d-none');
            }
            ...
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + pth[0] + '"]').find('table > tbody:first > tr:eq(' + ptempInd + ')').find('table[data-table-path="' + pth[0] + '.' + pth[1] + '"] > tbody:first > tr:eq(' + tempInd + ')').find('table[data-table-path="' + pth[0] + '.' + pth[1] + '.' + pth[2] + '"] > tbody > tr:eq(' + cindex + ')').find('input[data-path="'+ pth[0] + '.' + pth[1] + '.' + pth[2] +'.rowState"]:eq(0)').attr('value', 'removed');
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + pth[0] + '"]').find('table > tbody:first > tr:eq(' + ptempInd + ')').find('table[data-table-path="' + pth[0] + '.' + pth[1] + '"] > tbody:first > tr:eq(' + tempInd + ')').find('table[data-table-path="' + pth[0] + '.' + pth[1] + '.' + pth[2] + '"] > tbody > tr:eq(' + cindex + ')').find('input[data-path="'+ pth[0] + '.' + pth[1] + '.' + pth[2] +'.rowState"]:eq(0)').val('removed');
            ...
            //bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').remove();
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="'+ path +'.rowState:eq(0)"]').attr('value', 'removed');
            bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + path + '"]').find('table > tbody:first > tr:eq(' + tempInd + ')').find('input[data-path="'+ path +'.rowState:eq(0)"]').vale('removed');
        }
    }
            ...
            if ($this.closest('.detail-template-body').attr('data-display-picture') === '1') {
                ...
                var $appendTaxConfigHtml = '<div class="clearfix w-100"></div>' +
                                                '<span>' +
                                                    ...
                                                    $appendTaxHtml +
                                                '</span>';
                $parent.append($appendTaxConfigHtml).promise().done(function() {
                    Core.initBPInputType($parent);
                    if (typeof $this.attr('select2-editing') !== 'undefined' && $this.attr('select2-editing') === '1') {
                        setTaxonamyDetailKeyFunction_<?php echo $this->methodId; ?>(_this, $parent);
                    }
                });
            } else {
                ...
                $parent.append('<div class="clearfix w-100"></div><span>' + $appendSignatureHtml + '</span>').promise().done(function() {
                    Core.initBPInputType($parent);
                });
            }
        }
        ...
        if (hideParam !== 'hide') {
            if ($this.closest('.detail-template-body').attr('data-display-picture') === '1') {
                $parent.append('<div class="clearfix w-100"></div>' +
                    ...
                    if (typeof $this.attr('select2-editing') !== 'undefined' && $this.attr('select2-editing') === '1') {
                        var $issignature = 1;
                        setTaxonamyDetailFunction_<?php echo $this->methodId; ?>(_this, $parent);
                    }
                    Core.initBPInputType($parent);
                });
            } else {
    ...
    function appendTaxonamyFunction_<?php echo $this->methodId; ?>(tag, _this, dtlCombo, hideParam) {
        ...
        if ($this.closest('.detail-template-body').attr('data-display-picture') === '1') {
            ...
            $parent.append('<div class="clearfix w-100"></div><span><p></p><div class="float-left" style="margin-left: -72px;"><img src="assets/core/global/img/images.jpg" class="img-fluid rounded-circle imageSrc" style="border:1px #ccc solid;height:70px"></div>' + actionBtns_<?php echo $this->methodId; ?>('') + bp_window_<?php echo $this->methodId; ?>.find('.taxonamy-expression-tags').find('.' + tag + '-' + groupPath[0]).html() + '</span>').promise().done(function() {
                ...
                if (typeof $this.attr('select2-editing') !== 'undefined' && $this.attr('select2-editing') === '1') {
                    setTaxonamyFunction_<?php echo $this->methodId; ?>(_this, $parent);
                }
    ...
    function actionBtns_<?php echo $this->methodId; ?>(grouPath) {
        return '<div class="template-action-buttons btn-group btn-group-circle btn-group-xs btn-group-solid" style="margin-left: -9.8%; margin-top: 11%; float: left !important;">' +
            ...
            '<a href="javascript:;" style="padding: 2px 6px 2px 6px;" onclick="bpCopyPrevData(this, \'' + grouPath + '\');" title="Мэдээлэл хуулах" class="btn btn-xs purple-plum"><i class="fa fa-copy"></i></a> ' +
            '</div>';
    }
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        ...
        Core.unblockUI('#bp-window-<?php echo $this->methodId; ?>');
        console.clear();
        
        try {

            bp_window_<?php echo $this->methodId; ?>.find('.bprocess-table-dtl').each(function(pkey, table) {

                var $thisTable = $(table),
                    $groupPath = $thisTable.attr('data-table-path');
                var $staticHtml = bp_window_<?php echo $this->methodId; ?>.find('[data-dtl-template-path="' + $groupPath + '"]');
                var $processTableTr = $thisTable.find('.tbody:first > .bp-detail-row'),
                    $processTableTrLen = $processTableTr.length;

                var btn = bp_window_<?php echo $this->methodId; ?>.find('div[data-section-path="' + $groupPath + '"]').find('button.bp-add-one-row:first');

                if (btn.length) {
                    //btn.attr('onclick', btn.attr('onclick').replace('bpAddMainRow_', 'bpAddMainRowNtr_'));
                }
                
                if (typeof bp_window_<?php echo $this->methodId; ?>.find('[data-dtl-template-path="' + $groupPath + '"]').attr('data-dtl-template-widget') !== 'undefined') {
                    var $staticHtml = bp_window_<?php echo $this->methodId; ?>.find('[data-dtl-template-path="' + $groupPath + '"]'),
                        $loop = 0;
                        var $staticHtml0 = $($staticHtml[0]);
                        var $selectFound = $staticHtml0.find('select:eq(0)'),
                            $selectPath = $selectFound.attr('data-path');
                        var $picFindPath = 'span[data-display-picture="0"][data-dtl-template-widget="'+ $staticHtml.attr('data-dtl-template-widget') +'"]';
                        
                        for (var $index = 0; $index < $processTableTrLen; $index++) {
                            
                            var $paramVal = $($processTableTr[$index]).find('select[data-path="'+ $selectPath +'"]:eq(0)').val();

                            if ($paramVal) {
                                if ($index > 0) {
                                    $staticHtml0.children().first().children('a').addClass('triggerClick').trigger('click');
                                }
                                
                                var $selectorIndex = $staticHtml0.find('.detail-template-body-rows:eq('+$index+')');
                                var $setSelector = $selectorIndex.find('select[data-path="' + $selectPath + '"]:eq(0)');
                                
                                $setSelector.trigger("select2-opening", 'notdisabled');
                                $setSelector.attr("select2-editing", '1');
                                $setSelector.select2('val', $paramVal).trigger("change");
                                $setSelector.select2("close");
                                var $checkProcessTable = $($processTableTr[$index]).find('table.bprocess-table-subdtl:eq(0)'),
                                    $checkProcessTableTrDtl = $checkProcessTable.find('tbody:eq(0) > tr');
                                    
                                var $signatureSelector = bp_window_<?php echo $this->methodId; ?>.find($picFindPath + ' > .detail-template-body-rows:eq('+ $index +')');
                                $signatureSelector.each(function (sindex, row) {
                                    var $inputSelector = $(row).find('input');
                                    
                                    $inputSelector.each(function (si, sinput) {
                                        var $sparamVal = $($processTableTr[$index]).find('input[data-path="'+ $(sinput).attr('name') +'"]:eq(0)').val();
                                        if ($sparamVal) {
                                            //console.log($(sinput).attr('data-path') + '========' + $sparamVal);
                                            $(sinput).val($sparamVal);
                                        }
                                    });
                                });
                                
                                if ($checkProcessTableTrDtl.length > 0) {
                                    var $dtlTablePath = $checkProcessTable.attr('data-table-path');
                                    var $staticDtlHtml = $selectorIndex.find('p.detail-template-body-sub-rows[data-dtl-dtl-path="'+ $dtlTablePath +'"]');
                                    var $dtlstaticHtml0 = $($staticDtlHtml[0]);
                                    var $dSelectFound = $dtlstaticHtml0.find('select:eq(0)'),
                                        $dSelectPath = $dSelectFound.attr('data-path');
                                        
                                    for (var $dindex = 0; $dindex < $checkProcessTableTrDtl.length; $dindex++) {

                                        var $paramVal = $($checkProcessTableTrDtl[$dindex]).find('select[data-path="'+ $dSelectPath +'"]:eq(0)').val();
                                        if ($dindex > 0) {
                                            $dtlstaticHtml0.find('a:eq(0)').addClass('triggerClick').trigger('click');
                                        }
                                        
                                        var $dselectorIndex = $selectorIndex.find('.detail-template-body-sub-rows:eq('+$dindex+')');
                                        if ($paramVal) {
                                            $dselectorIndex.find('select[data-path="' + $dSelectPath + '"]').trigger("select2-opening", 'notdisabled');
                                            $dselectorIndex.find('select[data-path="' + $dSelectPath + '"]').attr("select2-editing", '1');
                                            $dselectorIndex.find('select[data-path="' + $dSelectPath + '"]').select2('val', $paramVal).trigger("change");
                                            $dselectorIndex.find('select[data-path="' + $dSelectPath + '"]').select2("close");
                                            
                                            /* key */
                                            
                                            var $signatureSelectorDtl = bp_window_<?php echo $this->methodId; ?>.find($picFindPath + ' > .detail-template-body-rows:eq('+ $index +') > .detail-template-body-sub-rows:eq('+ $dindex +')');
                                            $signatureSelectorDtl.each(function (sindexd, rowd) {
                                                var $inputSelectorDtl = $(rowd).find('input');

                                                $inputSelectorDtl.each(function (si, sdinput) {
                                                    //console.log($checkProcessTableTrDtl[$dindex]);
                                                    //console.log('input[data-path="'+ $(sdinput).attr('name') +'"]:eq(0)');
                                                    
                                                    var $sdparamVal = $($checkProcessTableTrDtl[$dindex]).find('input[data-path="'+ $(sdinput).attr('name') +'"]:eq(0)').val();
                                                    if ($sdparamVal) {
                                                        //console.log($(sdinput).attr('data-path') + '========' + $sdparamVal);
                                                        $(sdinput).val($sdparamVal);
                                                    }
                                                });
                                            });
                                            var $checkKeyProcessTable = $($checkProcessTableTrDtl[$index]).find('table.bprocess-table-subdtl:eq(0)'),
                                                $checkKeyProcessTableTrDtl = $checkKeyProcessTable.find('tbody:eq(0) > tr');
                                                
                                            if ($checkKeyProcessTableTrDtl.length > 0) {
                                                var $dtlKeyTablePath = $checkKeyProcessTable.attr('data-table-path');
                                                var $keyPathArr = $dtlKeyTablePath.split('.');
                                                var $staticDtlKeyHtml = $dselectorIndex.find('p.detail-template-body-subkey-rows[data-dtl-dtl-path="'+ $keyPathArr[0] + '.' + $keyPathArr[1] + $keyPathArr[2] +'"]');
                                                var $keydtlstaticHtml0 = $($staticDtlKeyHtml[0]);
                                                var $dkSelectFound = $keydtlstaticHtml0.find('select:eq(0)'),
                                                    $dkSelectPath = $dkSelectFound.attr('data-path');
                                                /* console.log($dselectorIndex);
                                                console.log('$dtlKeyTablePath = ' +$dtlKeyTablePath); */
                                                
                                                for (var $dkindex = 0; $dkindex < $checkKeyProcessTableTrDtl.length; $dkindex++) {

                                                    var $paramVal = $($checkKeyProcessTableTrDtl[$dkindex]).find('select[data-path="'+ $dkSelectPath +'"]:eq(0)').val();
                                                    
                                                    /* console.log($dkindex + '======== ' + $paramVal);
                                                    console.log($dkSelectPath + '======== ' + $paramVal); */
                                                    
                                                    if ($dkindex > 0) {
                                                        $keydtlstaticHtml0.find('a:eq(0)').addClass('triggerClick').trigger('click');
                                                    }
                                                    
                                                    var $dkselectorIndex = $selectorIndex.find('.detail-template-body-subkey-rows:eq('+$dkindex+')');
                                                    if ($paramVal) {
                                                        
                                                        $dkselectorIndex.find('select[data-path="' + $dkSelectPath + '"]').trigger("select2-opening", 'notdisabled');
                                                        $dkselectorIndex.find('select[data-path="' + $dkSelectPath + '"]').attr("select2-editing", '1');
                                                        $dkselectorIndex.find('select[data-path="' + $dkSelectPath + '"]').select2('val', $paramVal).trigger("change");
                                                        $dkselectorIndex.find('select[data-path="' + $dkSelectPath + '"]').select2("close");
                                                        
                                                        var $signatureSelectorKey = bp_window_<?php echo $this->methodId; ?>.find($picFindPath + ' > .detail-template-body-rows:eq('+ $index +') > .detail-template-body-subkey-rows:eq('+ $dindex +')');
                                                        $signatureSelectorKey.each(function (sindexd, rowd) {
                                                            var $inputSelectorKey = $(rowd).find('input');

                                                            $inputSelectorKey.each(function (si, kinput) {
                                                                var $kparamVal = $($checkKeyProcessTableTrDtl[$dkindex]).find('input[data-path="'+ $(kinput).attr('name') +'"]:eq(0)').val();
                                                                if ($kparamVal) {
                                                                    /* console.log($(kinput).attr('data-path') + '========' + $kparamVal); */
                                                                    $(kinput).val($kparamVal);
                                                                }
                                                            });
                                                        });
                                                        
                                                    } else {
                                                        $dkselectorIndex.addClass('hidden');
                                                    }
                                                }
                                            }
                                            ...
                                            $dselectorIndex.addClass('hidden');
                                        }
                                    }
                                }
                            }
                    ...
                    var $groupPathSplit = $groupPath.split('.');

                    if (typeof $groupPathSplit[0] !== 'undefined' && !(typeof bp_window_<?php echo $this->methodId; ?>.find('[data-dtl-template-path="' + $groupPathSplit[0] + '"]').attr('data-dtl-template-widget') !== 'undefined')) {
                        $loop = 0;
                        ...
                        for (var $index = 0; $index < $processTableTrLen; $index++) {
                            ...
                            if ($paramVal) {
                                ...
                                if ($setSelector.length < 1) {
                                    $setSelector = bp_window_<?php echo $this->methodId; ?>.find('.ntr_fullhtml').find('select[data-path="' + $selectPath + '"]:eq(0)');
                                }

                                if (($paramVal.toString()).indexOf(",") < 0) {
                                    $setSelector.trigger("select2-opening", [true]);
                                    $setSelector.attr("select2-editing-notemp", '1');
                                    $setSelector.select2('val', $paramVal).trigger("change");
                                    $setSelector.select2("close");
                                } else {  
                                    var $paramValArr = ($paramVal.toString()).split( "," );
                                    $setSelector.trigger("select2-opening", [true]);
                                    $setSelector.select2('val', $paramValArr).trigger("change");
                                    $setSelector.attr("select2-editing-notemp", '1');
                                    $setSelector.select2("close");
                                }
                            ...
                            if ($groupPathSplit[0] === 'NTR_SERVICE_RESPONSIBLE_DV') {
                                $thisTable.find('select').each(function (ins, srow) {
                                    var $subSelect2Selector = $(srow),
                                        $sValue = $subSelect2Selector.val(),
                                        $dataPathSelector = $subSelect2Selector.attr('data-path');
                                        
                                    if ($dataPathSelector !== $selectPath && $sValue) {
                                        var $splitDataPathSlec = $dataPathSelector.split('.');
                                        if ($splitDataPathSlec.length < 3) {
                                            if (typeof $subSelect2Selector.attr('data-edit-value') !== 'undefined' && $subSelect2Selector.attr('data-edit-value').indexOf(', ') > 0) {
                                                $sValue = $subSelect2Selector.attr('data-edit-value').split(', ');
                                            }

                                            $setSelector = bp_window_<?php echo $this->methodId; ?>.find('.ntr_fullhtml').find('select[data-path="' + $dataPathSelector + '"]:eq(0)');
                                            if (($paramVal.toString()).indexOf(",") < 0) {
                                                $setSelector.trigger("select2-opening", [true]);
                                                $setSelector.attr("select2-editing-notemp", '1');
                                                $setSelector.select2('val', $sValue).trigger("change");
                                                $setSelector.select2("close");
                                            } else {  
                                                var $paramValArr = ($paramVal.toString()).split( "," );
                                                $setSelector.trigger("select2-opening", [true]);
                        ...
                        var $staticHtml0 = $($staticHtml[0]);
                        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ $groupPathSplit[0] +'"] > tbody > tr:eq(0) > td > input').each(function(pkey, input) {
                            var $input = $(input),
                                $inputPath = $input.attr('data-path'),
                                $inputval = $input.val();
                        
                            if ($inputval) {

                                var $selectorIndex = $staticHtml0.find('.detail-template-body-rows:eq('+$index+')');
                                var $setSelector = $selectorIndex.find('input[data-path="' + $inputPath + '"]:eq(0)');
                                
                                if ($setSelector.length < 1) {
                                    $setSelector = bp_window_<?php echo $this->methodId; ?>.find('.ntr_fullhtml').find('input[data-path="' + $inputPath + '"]:eq(0)');
                                }
                                
                                $setSelector.attr("value", $inputval);
                                $setSelector.val($inputval);
                            }
                        });
            ...
            Core.unblockUI('#bp-window-<?php echo $this->methodId; ?>');
            $('.ntr_fullhtml').find('.triggerClick').removeClass('triggerClick');
        } catch (err) {
            console.log('::::::::::: begin editMode error :::::::::::');
            console.log(err);
            console.log('::::::::::: end editMode error :::::::::::');
            Core.unblockUI('#bp-window-<?php echo $this->methodId; ?>');
        }
    });
    
    function setTaxonamyDetailKeyFunction_<?php echo $this->methodId; ?>(_this, $parent) {
        ...
        var $this = $(_this),
            ...
            subIndex = $this.closest('.detail-template-body-subkey-rows').attr('data-index'),
            subKeyIndex = $this.closest('.detail-template-body-subkey-rows').attr('data-key-index');
        $this.attr('select2-editing', '0');
        var $mainSelector = bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] + '"] > tbody > tr:eq('+ cindex +')'),
            $detailMainSelector__ = $mainSelector.find('table[data-table-path="'+ groupPath[0] + '.'+ groupPath[1] + '"] > tbody > tr:eq('+ subIndex +')'),
            $keyMainSelector__ = $detailMainSelector__.find('table[data-table-path="'+ groupPath[0] + '.'+ groupPath[1] + '.'+ groupPath[2] + '"] > tbody > tr:eq('+ subKeyIndex +') > td > input');
            
        $keyMainSelector__.each(function(pkey, input) {
            var $input = $(input),
                $inputval = $input.val();
            
            if ($inputval) {
                var $splitInputPathArr = ($input.attr('data-path')).split('.');
                $parent.find('input[data-path="'+ groupPath[0] + '.' + $splitInputPathArr[$splitInputPathArr.length - 1]  +'"]:eq(0)').val($inputval);
            }
        });
        
        $detailMainSelector__.find('table[data-table-path="'+ groupPath[0] + '.'+ groupPath[1] + '.'+ groupPath[2] + '"] > tbody > tr:eq('+ subKeyIndex +') > td > select').each(function(pkey, select) {
            var $select = $(select),
                $selectval = $select.val();

            var $selectPath = $select.attr('data-path').split('.');
            if ($selectval && $selectPath[$selectPath.length -1] !== groupPath[3]) {

                var $setSelector = $parent.find('select[data-path="'+ $select.attr('data-path') +'"]:eq(0)');
                if ($setSelector.length < 1) {
                    $setSelector = $parent.find('select[data-path="'+ $selectPath[0] + '.' + $selectPath[$selectPath.length -1] +'"]:eq(0)');
                }

                $setSelector.trigger("select2-opening", [true]);
                $setSelector.select2('val', $selectval);
                $setSelector.select2("close");
            }
        });
        
        $parent.find('a.citizenRef_<?php echo $this->methodId; ?>').trigger('click');
    }
    
    function setTaxonamyDetailFunction_<?php echo $this->methodId; ?>(_this, $parent) {
        var $this = $(_this);
        var groupPath = $this.attr('data-path').split('.'),
            cindex = $this.closest('.detail-template-body-sub-rows').attr('data-parent-index'),
            subIndex = $this.closest('.detail-template-body-sub-rows').attr('data-index');
            
        var $mainSelector = bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] + '"] > tbody > tr:eq('+ cindex +')'),
            $detailMainSelector__ = $mainSelector.find('table[data-table-path="'+ groupPath[0] + '.'+ groupPath[1] + '"] > tbody > tr:eq('+ subIndex +') > td > input');
        $this.attr('select2-editing', '0');
        $detailMainSelector__.each(function(pkey, input) {
            var $input = $(input),
                $inputval = $input.val();
        ...
        $mainSelector.find('table[data-table-path="'+ groupPath[0] + '.'+ groupPath[1] + '"] > tbody > tr:eq('+ subIndex +') > td > select').each(function(pkey, select) {
            var $select = $(select),
                $selectval = $select.val();
            var $selectPath = $select.attr('data-path').split('.');
            if ($selectval && $selectPath[$selectPath.length -1] !== groupPath[2]) {
    ...
    function setTaxonamyFunction_<?php echo $this->methodId; ?>(_this, $parent) {
        ...
        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] +'"] > tbody > tr:eq('+ cindex +') > td > input').each(function(pkey, input) {
            var $input = $(input),
                $inputval = $input.val();
                
            if ($inputval) {
                if ($parent.find('input[data-path="'+ $input.attr('data-path') +'"]:eq(0)').length > 0) {
                    $parent.find('input[data-path="'+ $input.attr('data-path') +'"]:eq(0)').val($inputval);
                } else {
                    bp_window_<?php echo $this->methodId; ?>.find('.ntr_fullhtml').find('input[data-path="' + $input.attr('data-path') + '"]:eq(0)').val($inputval);
                }
            }
        });
        
        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] +'"] > tbody > tr:eq('+ cindex +') > td > select').each(function(pkey, select) {
            var $select = $(select),
                $selectval = $select.val();

            var $selectPath = $select.attr('data-path').split('.');
            if ($selectval && $selectPath[$selectPath.length -1] !== groupPath[1]) {
                ...
                if ($setSelector.length < 1) {
                    $setSelector = bp_window_<?php echo $this->methodId; ?>.find('.ntr_fullhtml').find('select[data-path="' + $select.attr('data-path') + '"]:eq(0)');  
                }
    ...
    function setTaxonamyFunctionNoTemp_<?php echo $this->methodId; ?>(_this, $parent) {
        
        var groupPath = $(_this).attr('data-path').split('.'),
            $this = $(_this),
            cindex = $this.closest('.detail-template-body-rows').index();
        $this.attr('select2-editing-notemp', '0');
        ...
        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] +'"] > tbody > tr:eq('+ cindex +') > td > .dateElement > input').each(function(pkey, input) {
            var $input = $(input),
                $inputval = $input.val();
            if ($inputval) {
                $parent.find('input[data-path="'+ $input.attr('data-path') +'"]:eq(0)').val($inputval);
        ...
        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] +'"] > tbody > tr:eq('+ cindex +') > td > textarea').each(function(pkey, input) {
            var $input = $(input),
                $inputval = $input.val();
            if ($inputval) {
                $parent.find('textarea[data-path="'+ $input.attr('data-path') +'"]:eq(0)').val($inputval);
            }
        });
        ...
        bp_window_<?php echo $this->methodId; ?>.find('table[data-table-path="'+ groupPath[0] +'"] > tbody > tr:eq('+ cindex +') > td > select').each(function(pkey, select) {
            ...
            if ($selectval  && $selectPath[$selectPath.length -1] !== groupPath[1]) {
                $parent.find('select[data-path="'+ $select.attr('data-path') +'"]:eq(0)').select2('val', $selectval).trigger('change');
            }
        });
    ...
    <?php if (!$this->renderType) { ?>
        ...
        $(function() {
            ...
            $('body').on('click', '.renderBpTemplate', function(e) {
                ...
                $this.remove();
                $('li[trg-metadataid="' + $metadataId + '"]', '.mainbp-window-<?php echo $this->methodId; ?>').removeClass('hidden');
            });
