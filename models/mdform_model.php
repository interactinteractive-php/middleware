...
class Mdform_Model extends Model {
    ...
    public function isCheckSystemTable($tblName, $isTblCreated = false) {
        ...
        /*if ($tblName != '' 
            && strpos($tblName, 'v_') === false 
            && strpos($tblName, 'd_1') === false 
            && strpos($tblName, 'kpi_') === false 
            && strpos($tblName, 'cam_') === false) {
    ...
    public function saveKpiDynamicDataModel($sourceRecordId = null, $postArrData = array()) {
        ...
        try {
            ...
            $kpiTblIdField  = $fieldConfig['idField'] ? $fieldConfig['idField'] : 'ID';
            ...
            if (!isset($postData['isMicroFlow']) && !$isExcelImport) { 
                ...
                if ($expIndicatorId) {
                    $microFlowResponce = (new Mdexpression())->executeMicroFlowExpression($expIndicatorId, $dataClob);            
            ...
            if (Input::postCheck('wfmStatusParams')) {
    ...
    public function kpiSaveMetaDmRecordMap($srcIndicatorId, $srcRecordId, $saveData = array()) {
        ...
        if (Input::numeric('isKpiComponent') == 1) {
            ...
            try {
                ...
                if ($trgIndicatorIds = issetParam($metaDmRecordMaps['indicatorId'])) {
                                ...
                                foreach ($createFieldDataMapConfigs as $dataMapConfig) {
                                    ...
                                    if ($dataMapConfigCode) {
                                        ...
                                        if ($checkCodeValue == $dataMapConfigCode) {
                                            ...
                                            if ($company_department_id = issetParam($childRowData['company_department_id'])) {
                                                $metaDmRecordMapsSubForm[$k][$dataMapConfigMetaInfoIndicatorId.'_'.$subUniqId]['COMPANY_DEPARTMENT_ID'] = $company_department_id;
                                            }
                                        }
                                    }
                                }
                            ...
                            foreach ($addonForm as $addonIndicatorId => $addonParams) {
                                ...
                                if (!$addonRecordId && !isset($checkDeletedRows[$trgIndicatorId.'_'.$addonIndicatorId])) {
                                    ...
                                    if ($checkRecordMapData) {
                                        ...
                                        $trgMapIds = Arr::implode_key(',', $checkRecordMapData, 'ID', true);
                                        $trgRecordIds = Arr::implode_key(',', $checkRecordMapData, 'TRG_RECORD_ID', true);

                                        $this->db->Execute("DELETE FROM META_DM_RECORD_MAP WHERE ID IN ($trgMapIds)");
                                        $this->db->Execute("DELETE FROM $addonIndicatorTblName WHERE ID IN ($trgRecordIds)");
                                    }
                                ...
                                if ($result['status'] != 'success') {
                                    ...
                                    if ($childConfigSubRows) {
                                        ...
                                        if ($rowCompanyDepartmentId = issetParam($addonParams['COMPANY_DEPARTMENT_ID'])) {
                                            $findHeaderDataFilter = " AND COMPANY_DEPARTMENT_ID = $rowCompanyDepartmentId";
                                        }
            ...
            if ($drillDownCriteria) {
                ...
                if (is_array($defaultCriteriaData)) {
                    foreach ($defaultCriteriaData as $drillColumn => $drillVal) {
                        if ($drillVal != '') {
                            $subCondition .= " AND $drillColumn = '".Input::param($drillVal)."'";
                            $isSubCondition = true;
                        }
                    }
                }
    ...
    public function filterKpiIndicatorValueChartModel() {
        ...
        try {
            ...
            foreach ($columns as $col) {
                ...
                if ($columnName == $category && ($showType == 'combo' || $showType == 'popup' || $showType == 'radio')) {
            ...
            $valueAliasCol = ($isBuild == '1') ? 'NAME' : $category;
            ...
            if ($isCategoryCombo) {
                ...
                if ($chartType == 'card' || $chartType == 'card_vertical') {
                    ...
                    $valueSelectCol = $category . ' AS NAME, ' . $category . ' AS VALUE' ;
                    ...
                    if ($isCategoryGroupCombo) {
                        ...
                        $sql = "
                            ...
                            WHERE $category IS NOT NULL 
                                AND ".$categoryGroup."_DESC IS NOT NULL 
                                $subCondition 
                            GROUP BY $category, $categoryGroup, ".$categoryGroup."_DESC";
    ...
    public function createKpiDmChartModel() {
        ...
        if ($chartIndicatorId = Input::numeric('chartIndicatorId')) {
            ...
            $this->db->AutoExecute('KPI_INDICATOR', array(
                    'NAME' => $param['name']
                ), 
                'UPDATE', 
                'ID = '.$chartIndicatorId
    ...
    public function saveBlockExpressionModel() {
        ...
        $getParamInput = self::getKpiIOIndicatorColumnsModel($param['id'], 1);
        $getParamOut = self::getKpiIOIndicatorColumnsModel($param['id']);

        // $varFncExpressionString = 'function generatedFunction(';

        // if ($getParamInput) {
        //     foreach ($getParamInput as $row) {
        //         $varFncExpressionString .= $row['COLUMN_NAME'].',';
        //     }
        // }

        // $varFncExpressionString = rtrim($varFncExpressionString, ',');
        // $varFncExpressionString .= '){'.PHP_EOL.$param['varFncExpressionString'];

        // if ($getParamOut) {
        //     $pushReturn = '{';
        //     foreach ($getParamOut as $row) {
        //         $pushReturn .= '"'.$row['COLUMN_NAME'].'":'.$row['COLUMN_NAME'].',';
        //     }
        //     $pushReturn .= '}';
        //     $varFncExpressionString .= PHP_EOL.'return '.$pushReturn;
        // }

        // $varFncExpressionString .= PHP_EOL.'}';
...
//            ORDER BY KIIM.ORDER_NUMBER ASC";
    ...
    public function mainChartTypeDataModel() {
        ...
        $qry = "SELECT DISTINCT
                    ...
                    LEFT JOIN (
                            SELECT 
                                ...
                                t2.ICON,
                                t2.PARENT_ID,
                                LISTAGG( t4.CODE,'##') WITHIN GROUP ( ORDER BY t4.CODE) AS SUB_TYPE_CODE
                            FROM KPI_TYPE t0 
                            INNER JOIN KPI_TYPE t1 ON t0.ID = t1.PARENT_ID
