...
var ChartsAmcharts = function () {
    ...
    var gauge = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
    ...
    var angularGauge = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
    ...
    var treedFunnel = function (container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId) {
        var chart = AmCharts.makeChart(container, {
            "type": "funnel",
            "theme": "light",
            "dataProvider": data,
    ...
    var stackedColumn = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
    ...
    var stacked = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var chartConfigs = {
            ...
            "graphs": config.graphs,
            "rotate": config.addonSettings && config.addonSettings.isvertical == '1' ? false : true,
            "categoryField": config.xAxisName,
            "categoryAxis": {
                "gridPosition": "start",
                "axisAlpha": 0,
                ...
                "labelFunction": function (label, item, axis) {
                    if (label.length > config.labelTextSubStr)
                        return html_entity_decode(label.substr(0, config.labelTextSubStr)) + '...';
        ...
        if (parseFloat(drillDown) > 0 && typeof noDrillDown == 'undefined') {
            chart.addListener("clickGraphItem", function (event) {
                isSubChart = 1;
                selectedChartEvent = event;
                if (config.hasOwnProperty('rowData')) {
                    event.rowData = config.rowData;
                }
                ChartsAmcharts.subChartInit(metaDataId, event, $('.dashboard-filter-form-' + mainMetaDataId).serialize(), mainMetaDataId);
            });
        }
    ...
    var serial = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        ...
        function zoomChart() {
            serial.zoomToIndexes(serial.dataProvider.length - 40, serial.dataProvider.length - 1);
        }
    ...
    var radar = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var resultData = $.map(data, function (item) {
            if (item.percent) {
                return {
                    ...item,
                    [config.xAxisName]: item[config.xAxisName]+'|'+item.percent
                };
            } else
                return item;
        })        
        ...
        var color = data && data[0]['color'] ? [data[0]['color']] : [];
    ...
    var column = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        ...
        if (typeof config.templateWidth !== 'undefined' && config.templateWidth) {
            series.columns.template.width = config.templateWidth;
        }
...
//        }
    ...
    var pie_charts_bullets = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var columnConfig = {
            "data": data,
            "hiddenState": {
              "properties": {
    ...
    var risk_heatmap = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {

        if (config.valueAxisTitle && config.categoryAxisTitle) {
            if ($('#'+container).parent().find('.heatmap-y-text').length) {
                $('#'+container).removeClass('heatmap-xy-text');
                $('#'+container).parent().find('.heatmap-y-text').remove();
                $('#'+container).parent().find('.heatmap-x-text').remove();
            }
            $('#'+container).addClass('heatmap-xy-text');
            $('#'+container).parent().append('<div class="heatmap-y-text" style="text-align:center;transform: rotate(-90deg);-webkit-transform: rotate(-90deg);-moz-transform: rotate(-90deg);-ms-transform: rotate(-90deg);-o-transform: rotate(-90deg);top: 50%;left: -20px;position: absolute;">'+config.categoryAxisTitle+'</div>'+
            '<div class="heatmap-x-text" style="text-align:center;border-top: 1px solid #9c9c9c;">'+config.valueAxisTitle+'</div>');
        }
    ...
    var percent_stacked_area_chart = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        am4core.useTheme(am4themes_animated);
    ...
    var variable_radius_radar = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        am4core.useTheme(am4themes_animated);
        ...
        var categoryAxis = chart.xAxes.push(new am4charts.CategoryAxis());
        categoryAxis.dataFields.category = config.addonSettings.category.toUpperCase();
        ...
        for (var saco = 0; saco < getLegend.length; saco++) {
            ...
            series1.dataFields.categoryX = config.addonSettings.category.toUpperCase();
    ...
    var animated_xy_bubble = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        ...
        var sliderAnimation = slider.animate({ property: "start", to: 1 }, 40000, am4core.ease.linear).pause();
        ...
        slider.events.on("rangechanged", function() {
          ...
          for (var i = 0; i < series.data.length; i++) {
            ...
            dataContext.x = -56;
            dataContext.y = 62;
            dataContext.radius = dataContext[config.addonSettings.bubbleValue];
          }
    ...
    var cylinder = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var valAxis = [{"fontSize": chartCategoryAxisFontSize, "labelsEnabled": false}];
    ...
    var barAxis = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var chart = AmCharts.makeChart(container, {
            "type": "serial",
            "theme": "light",
            "categoryField": config.xAxisName,
    ...
    var durationOnvalueAxis = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var valueTitle = config.valueAxisTitle ? config.valueAxisTitle.split('_') : 'Title X';
        var catTitle = config.categoryAxisTitle ? config.categoryAxisTitle.split('_') : 'Title Y';
    ...
    var durationOnvalueAxis2 = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var valueTitle = config.valueAxisTitle ? config.valueAxisTitle.split('_') : 'Title X';
        var catTitle = config.categoryAxisTitle ? config.categoryAxisTitle.split('_') : 'Title Y';        
    ...
    var multipleValueAxis = function (container, data, config, metaDataId, drillDown, mainMetaDataId) {
        var chart = AmCharts.makeChart(container, {
            "type": "serial",
            "theme": "light",
            "categoryField": config.xAxisName,
    ...
    var donut = function (container, data, config, metaDataId, drillDown, mainMetaDataId, responseData) {
        ...
        if (config.addonSettings && config.addonSettings.labelmarkerwidth && config.addonSettings.labelmarkerheight) {
            var markerTemplate = chart.legend.markers.template;
            markerTemplate.width = config.addonSettings.labelmarkerwidth;
            markerTemplate.height = config.addonSettings.labelmarkerheight;        
        }        
...
//
    ...
    var clustered_column_line = function (container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId) {
        ...
        // Create axes
        var categoryAxis = chart.xAxes.push(new am4charts.CategoryAxis());
        categoryAxis.dataFields.category = data.xAxisName;
        categoryAxis.renderer.grid.template.location = 0;
        categoryAxis.renderer.minGridDistance = 5;

        var valueAxis = chart.yAxes.push(new am4charts.ValueAxis());
        var columnName = (data.columnName).split(',');
        var lineName = (data.lineName).split(',');
        var columnArray = data.columnArray;
        
        for (var c in columnName) {
            // Create series
            var series = chart.series.push(new am4charts.ColumnSeries());
            series.dataFields.valueY = columnName[c];
            series.dataFields.categoryX = data.xAxisName;
            series.columns.template.tooltipText = "[#fff font-size: 12px]{name} / {categoryX}:\n[/][#fff font-size: 13px]{valueY}[/] [#fff][/]";
            series.name = columnArray[columnName[c]];
        }
        ...
        for (var l in lineName) {
            // quantity line series
            var lineSeries = chart.series.push(new am4charts.LineSeries());

            lineSeries.tooltipText = "{valueY}";
            lineSeries.dataFields.categoryX = data.xAxisName;
            lineSeries.dataFields.valueY = lineName[l];
            //lineSeries.yAxis = valueAxis2;
            lineSeries.fill = lineSeries.stroke;
            lineSeries.strokeWidth = 2;
            lineSeries.snapTooltip = true;
            lineSeries.name = columnArray[lineName[l]];

            var CircleBullet = lineSeries.bullets.push(new am4charts.CircleBullet());
            var labelBullet = lineSeries.bullets.push(new am4charts.LabelBullet());
            labelBullet.label.text = "{"+lineName[l]+"}";
    ...
    var clustered = function (container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId) { 
    ...
    var clusteredHorizontal = function (container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId) {
    ...
    var drawChartAmchart = function (defaultCriteriaData, chartType, metaDataId, callback, workSpaceParams, workSpaceId, criteriaPosition) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {metaDataId: metaDataIdSplited[0], defaultCriteriaData: defaultCriteriaData, workSpaceParams: workSpaceParams, workSpaceId: workSpaceId, version: 1},
            beforeSend: function () {
            ...
            success: function (response) {
                ...
                $('#dashboard-title-' + metaDataId).attr('title', response.title).html(response.title);
                if (response.height) {
                    $('#dashboard-' + metaDataId).height(response.height);
                }

//                if (criteriaPosition !== 'top' && criteriaPosition !== 'topFilterButton') {
//                    $('.dashboard-filter-form-' + metaDataId).html(response.defaultCriteria);
//                }
                var config = ChartsAmcharts.getConfig(response, metaDataId, chartType);
                ChartsAmcharts.init(chartType, 'dashboard-' + metaDataId, config.data, config, response.linkedMetaDataId, response.DRILLDOWN, metaDataId, response);
    ...
    var getConfig = function (response, metaDataId, chartType) {
        var data = [];
        if (typeof response.series.data !== 'undefined' && Object.keys(response.series.data).length > 0) {
        ...
        if (response.isTitle == 1) {
            ...
            if (typeof response.description != 'undefined' && response.description != '' && response.description != null) {
                $('#dashboard-helper-' + metaDataId).html(response.description);
                $('#dashboard-helper-' + metaDataId).attr('title', response.description);
            } else {
        ...
        if (typeof response.series.xAxisGroupName !== 'undefined') {
            ...
            var legendFormat = (typeof response.legendFormat !== 'undefined' && response.legendFormat) ? response.legendFormat : "[[category]]: <b>[[value]]</b>";
        ...
        switch (response.chartType) {
            ...
            case 'am_radar_chart' :
            {
                ...
                if (yAxisArray.length == 2 && typeof yAxisArray[1] !== 'undefined' && (yAxisArray[1]).indexOf('_') !== -1) {
                    var yAxisArray3 = yAxisArray[1].split('_');
                    title = yAxisArray3[0];
                    valueField = yAxisArray3[1];
                }
            ...
            case 'am_3d_stacked_column_chart' :
            {
                ...
                $.each(yAxisArray, function (key, yAxisSingle) {
                    ...
                    var yAxisValueField = yAxisSingle.replace(yAxisSingleSpited[0] + '_', '');
                    tempObject = {
                        "lineAlpha": "0.2",
                        "type": "column",
                        "fillAlphas": 0.8,
            ...
            case 'am_trend_lines' :
            {
                var yAxisArray = chartTitleSeparatorSplit(response.series.yAxisName);
                graphs = [{
                        "balloonText": "[[category]]<br><b><span style='font-size:10px;'>value:[[value]]</span></b>",
                        "bullet": "round",
                        "dashLength": 3,
                        "fontSize": chartValueFontSize,
                        "colorField": "color",
                ...
                if (typeof response.series.xAxisGroupValue != 'undefined' && (response.series.xAxisGroupValue).length != 0)
            ...
            case 'am_stacked_bar_chart' :
            {
                    ...
                    if (themeColor.length > 0) {
                        tempObject = {
                            ...
                            "labelFunction": function (item) {
                                return rtrim(rtrim(rtrim(number_format(Math.abs(item.values.value), '2', '.', ','), '0'), '0'), '.');
                            }
                        };
                    } else {
                        tempObject = {
                ...
                if (response.addonSettings && response.addonSettings.stackx && response.addonSettings.stacky) {
            ...
            {
                var yAxisArray = chartTitleSeparatorSplit(response.series.yAxisName);
                var valueTitle = response.valueAxisTitle ? response.valueAxisTitle.split('_') : '';
                var catTitle = response.categoryAxisTitle ? response.categoryAxisTitle.split('_') : 'Title Y';
                ...
                if (yAxisArray.length === 2) {
                    ...
                    var yAxisValueField = yAxisArray[0].replace(yAxisSingleSpited[0] + '_', '');
                    var yAxisValueField1 = yAxisArray[1].replace(yAxisSingleSpited1[0] + '_', '');                    
            ...
            case 'durarion_onvalue_axis2' :
                {
                    var yAxisArray = chartTitleSeparatorSplit(response.series.yAxisName);
                    var valueTitle = response.valueAxisTitle ? response.valueAxisTitle.split('_') : 'Title X';
                    var catTitle = response.categoryAxisTitle ? response.categoryAxisTitle.split('_') : 'Title Y';
                        valueTitle = valueTitle.length === 2 ? valueTitle[1] : valueTitle[0];
                        catTitle = catTitle.length === 2 ? catTitle[1] : catTitle[0];
                    var getColors = '#8E44AD #3faba4 #ff8d00 #95A5A6 #d05454 #f3c200 #8775a7 #009c02 #b1e10a #e25f9e #e7bda2 #0041c4 #ff8d00 #ff0000 #2585ae #4e8539 #6ba6d4 #fdff00 #00ff04 #0085ff #d40a78 #b4c7f0 #da70d6 #edc613 #8E44AD #3faba4 #ff8d00 #95A5A6 #d05454 #f3c200 #8775a7 #009c02 #b1e10a #e25f9e #e7bda2 #0041c4 #ff8d00 #ff0000 #2585ae #4e8539 #6ba6d4 #fdff00 #00ff04 #0085ff #d40a78 #b4c7f0 #da70d6 #edc613 #8E44AD #3faba4 #ff8d00 #95A5A6 #d05454 #f3c200 #8775a7 #009c02 #b1e10a #e25f9e #e7bda2 #0041c4 #ff8d00 #ff0000 #2585ae #4e8539 #6ba6d4 #fdff00 #00ff04 #0085ff #d40a78 #b4c7f0 #da70d6 #edc613'.split(' ');
                    ...
                    if (yAxisArray.length === 2) {
                        ...
                        var yAxisValueField2 = yAxisArray[2].replace(yAxisSingleSpited2[0] + '_', '');
                        var yAxisValueField3 = yAxisArray[3].replace(yAxisSingleSpited3[0] + '_', '');
            ...
            case 'multiple_value_axis' :
            {
                ...
                graphs = [{
                        ...
                        "bulletBorderThickness": 1,
                        "hideBulletsCount": 30,
                        "title": yAxisArray[0].split('_')[0],
                        "valueField": yAxisArray[0].split('_')[1],
                        "fillAlphas": 0
                    }, {
                        "valueAxis": "v2",
                        "lineColor": "#FCD202",
                        ...
                        "bulletBorderThickness": 1,
                        "hideBulletsCount": 30,
                        "title": yAxisArray[1].split('_')[0],
                        "valueField": yAxisArray[1].split('_')[1],
                        "fillAlphas": 0
                    }, {
                        "valueAxis": "v3",
                        "lineColor": "#B0DE09",
                        ...
                        "bulletBorderThickness": 1,
                        "hideBulletsCount": 30,
                        "title": yAxisArray[2].split('_')[0],
                        "valueField": yAxisArray[2].split('_')[1],
                        "fillAlphas": 0
                    },{
                        "valueAxis": "v3",
                        "lineColor": "#B0DE09",
                        ...
                        "bulletBorderThickness": 1,
                        "hideBulletsCount": 30,
                        "title": yAxisArray[3] ? yAxisArray[3].split('_')[0] : '',
                        "valueField": yAxisArray[3] ? yAxisArray[3].split('_')[1] : '',
                        "fillAlphas": 0
                    },
                    {
                        "valueAxis": "v3",
                        ...
                        "bulletBorderThickness": 1,
                        "hideBulletsCount": 30,
                        "title": yAxisArray[4] ? yAxisArray[4].split('_')[0] : '',
                        "valueField": yAxisArray[4] ? yAxisArray[4].split('_')[1] : '',
                        "fillAlphas": 0
                    }
                ];
                break;
        ...
        var responseData = {
            ...
            "templateWidth": (typeof response.templateWidth !== 'undefined' && response.templateWidth ? response.templateWidth : ''),
            ...
            "isLegend": (typeof response.isLegend !== 'undefined') ? response.isLegend : '1', 
            "colorsSet": (typeof response.colorsSet !== 'undefined') ? response.colorsSet : null,
            "addonSettings": response.addonSettings
        };
        
        if (typeof response.series.rowData !== 'undefined' && Object.keys(response.series.rowData).length > 0) {
            responseData.rowData = response.series.rowData;
        }
    ...
    var subChartInit = function (metaDataId, event, defaultCriteriaData, mainMetaDataId) {
        ...
        if (event.hasOwnProperty('rowData') && (event.graph).hasOwnProperty('valueField') && event.graph.valueField) {
            var valueField = event.graph.valueField;
            var categoryField = event.item.category;
            var rowDatas = event.rowData;
            
            if (rowDatas.hasOwnProperty(categoryField + '_' + valueField)) {
                var rowData = event.rowData[categoryField + '_' + valueField];
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (typeof response.metaType !== 'undefined' && response.metaType != 'metagroup') {

                    if ($('[id^=dashboard-' + response.linkedMetaDataId + '_]').length) {
                        var dashUniqId = $('[id^=dashboard-' + response.linkedMetaDataId + '_]').eq(0).attr('id').split('_');
                        mainMetaDataId = response.linkedMetaDataId + '_' + dashUniqId[1];
                    }

                    $('#dashboard-' + mainMetaDataId).width(response.width);
                    $('#dashboard-title-' + mainMetaDataId).attr('title', response.title).html(response.title);
                    ...
                    $('.dashboard-filter-form-' + metaDataId).html(response.defaultCriteria);
                    if (typeof response.series.yAxisName != 'undefined' && response.series.yAxisName != false) {
                    } else {
                        ChartsAmcharts.drawChartAmchart(_defaultCriteriaData, _chartType, _metaDataId);
                        Core.unblockUI();
                        return false;
                    }
                    var subconfig = ChartsAmcharts.getConfig(response, metaDataId, response.chartType);
                    ChartsAmcharts.init(response.chartType, 'dashboard-' + mainMetaDataId, subconfig.data, subconfig, response.linkedMetaDataId, response.DRILLDOWN, mainMetaDataId, response);
                    ...
                    if (response.hasOwnProperty('showType') && response.showType == 'tab') {

                        appMultiTabByContent({ metaDataId: response.linkMetaDataId, title: response.listName, type: 'dataview', content: response.data });
                        ...
                        if (typeof dataGridLink !== 'undefined' && typeof response.drillCriteria !== 'undefined' && response.drillCriteria) {
                            ...
                            dvSearchParam['drillDownDefaultCriteria'] = JSON.stringify(response.drillCriteria);

                            dataViewLoadByElement(dataGridLink, dvSearchParam);
                        
                        } else {
                            $('#dashboard-' + mainMetaDataId).html('<div class="col-md-12 main-dataview-container pl0 pr0" id="object-value-list-' + response.linkMetaDataId + '">' + response.data + '</div>');
                            $('.remove-type-' + response.linkMetaDataId).remove();
                            $('.div-objectdatagrid-' + response.linkMetaDataId).addClass('pl0 pr0').attr('style', 'height: 350px !important;');
                            $('#objectdatagrid-' + response.linkMetaDataId).attr('style', 'height: 350px !important;').attr('height', '350');
                        }
                    }
                }
    ...
    var gantt = function (container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId, responseData) {
        var chart = new AmCharts.makeChart(container, {
            "type": "gantt",
            "theme": "light",
            "marginRight": 70,
    ...
    return {
        init: function (dataChart, container, data, config, metaDataId, drillDown, mainMetaDataId, responseData) {

            switch (dataChart) {
                case 'am_serial' :
                {
                    serial(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_radar_chart' :
                {
                ...
                case 'am_column' :
                {
                    column(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'pie_charts_bullets' :
                {
                    pie_charts_bullets(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'risk_heatmap' :
                {
                    risk_heatmap(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'animated_xy_bubble' :
                {
                    animated_xy_bubble(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'percent_stacked_area_chart' :
                {
                    percent_stacked_area_chart(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'variable_radius_radar' :
                {
                    variable_radius_radar(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_threed_cylinder_chart' :
                {
                    cylinder(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_bar' :
                {
                ...
                case 'am_bar_axis' :
                {
                    barAxis(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'durarion_onvalue_axis' :
                {
                    durationOnvalueAxis(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'durarion_onvalue_axis2' :
                {
                    durationOnvalueAxis2(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'multiple_value_axis':
                {
                    multipleValueAxis(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_donut' :
                {
                    donut(container, data, config, metaDataId, drillDown, mainMetaDataId, responseData);
                    break;
                }
                case 'am_pie' : {
                    pie(container, data, config, metaDataId, drillDown, mainMetaDataId, responseData);
                    break;
                }
                case 'am_dual' :
                {
                    dual(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_gantt' :
                {
                    gantt(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId, responseData);
                    break;
                }
                case 'clustered_bar_chart' :
                {
                    clustered(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'clustered_bar_chart_horizontal' :
                {
                    clusteredHorizontal(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_stacked_bar_chart' :
                {
                    stacked(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_reversed' : {
                    reverse(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId, responseData);
                    break;
                }
                case 'am_zoomable_value_axis' :
                {
                    zoomable(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_trend_lines' :
                {
                    trendLine(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_threed_funnel' :
                {
                    treedFunnel(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_funnel' :
                {
                    funnel(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_cylinder_gauge' :
                {
                    cylinderGauge(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_angular_gauge' :
                {
                    angularGauge(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_combined_bullet' :
                {
                    combined(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_mixed_dialy_and_intra_day_chart' :
                {
                    mixedDialy(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_3d_stacked_column_chart' :
                {
                    threeDStackedClustered(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_3d_stacked_column_chart_2' :
                {
                    stackedColumn(container, data, config, metaDataId, drillDown, mainMetaDataId);
                    break;
                }
                case 'am_clustered_column_line' :
                {
                    clustered_column_line(container, data, config, metaDataId, dataChart, drillDown, mainMetaDataId);
                    break;
                }
            }
        },

        drawChartAmchart: function (defaultCriteriaData, chartType, metaDataId, callback, workSpaceParams, workSpaceId, criteriaPosition) {
            drawChartAmchart(defaultCriteriaData, chartType, metaDataId, callback, workSpaceParams, workSpaceId, criteriaPosition);
        },
        ...
        subChartInit: function (metaDataId, event, defaultCriteriaData, mainMetaDataId) {
