...
$(function () {
    ...
    setIcon = function (elem, refStructureId, dataViewId) {
        var _left = elem['LEFT'];
        ...
        var html = '<div id="' + elem['ID'] + '" ' +
                ...
                'data-dobpid="' + elem['ID'] + '" ' +
                'style="width: ' + (elem['TYPE'] == 'circle' ? '30px' : 'auto') + '; height:' + (elem['TYPE'] == 'circle' ? '30px' : 'auto') + '; border-color: ' + elem['WFM_STATUS_COLOR'] + '; background: '+(elem['IS_ACTIVE'] == '1' ? '#28df99' : (elem['IS_WORKED'] == '1' || elem['IS_START'] == '1' ? '#f5a25d' : '#ccc'))+'; padding: 6px 14px;border-radius: 25px !important;"' +
                '>';
        html += '<span class="iconText" style="position: inherit;">';
        if (elem['TYPE'] == 'rectangle') {
            html += '<div class="bp-name" style="color:#000;font-size: 12px;">' + elem['WFM_STATUS_NAME'] + '</div>';
        }
        html += '</span>';
        html += '</div>';
...
function viewVisualHtmlMetaData(mainBpId, transId, selftype, selectedRow, refStructureId, dataViewId) {
    if (mainBpId != '') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (typeof data.transitionId != 'undefined') {                
                    data.transitionId = transId;
                }
                workFlowDrawHtml(mainBpId, data, transId, selftype, selectedRow, refStructureId, dataViewId);
                if (data.status === 'success') {
...
function clickSeeSidebarFnc(element) {
    if ($(element).data('isactive') == '1') {
        var $wfmStatusId = $(element).attr('id');
        if (isNaN(parseFloat($wfmStatusId))) {
            return false;
        }
        tempWfmStatusId = $wfmStatusId;
        changeWfmStatusId(element, tempWfmStatusId, $(element).data('dataviewid'), $(element).data('refstructureid'), $(element).data('status-color'), $(element).data('status-name'), undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, function(){
            console.log('AAA');
        });
    } else {
        PNotify.removeAll();
        new PNotify({
            title: 'Warning',
            text: '<strong>' + $(element).data('status-name') + '</strong> - төлөвийг өөрчлөх боломжгүй байна.',
            type: 'warning',
            sticker: false
        });        
    }
...
function workFlowDrawHtml(mainBpId, data, transId, selftype, selectedRow, refStructureId, dataViewId) {
    
    $('#metaProcessDetial').html('');
    var _mainWindowHeight = $(window).height() - 143;
    var mainHtml = ''
            + '<div class="col pl0 d-none">'
                ...
                + '</div>'
            + '</div>'
            + '<div class="col-md-6 pr0 d-none" style="width: 430px; min-width: 430px; max-width: 430px;">'
                + '<div class="mb10 btn-group btn-group-devided meta-toolbar mt0 ph0 border-0">'
            ...
            + '<div class=" pivotgrid-table-left-cell d-none" style="width: 250px !important; min-width: 250px !important; max-height: ' + _mainWindowHeight + 'px; !important; height: ' + _mainWindowHeight + 'px; border: 1px solid #999;">'
            ...
            + '<div class="css-editor" id="workFlowEditor" style="width:100%;float: left;height: 100%;"></div>'
            + '</div>'
            + '</div>'
            + '</div>'
            + '<div class="pivotgrid-table-center-right-collapse-cell d-none"></div>'
            + '<div class="pivotgrid-table-center-right-cell d-none" data-status="open" style="border: 1px solid #999; margin-top:0px; max-height: ' + _mainWindowHeight + 'px; height: ' + _mainWindowHeight + 'px; display: table-cell; vertical-align: top; margin: 0; padding: 0 0 0 10px; width: 400px; min-width: 400px; max-width: 400px;">'
    ...
    selectJsPlumbViewBy(_selectTransitionId, selectedRow, refStructureId, dataViewId);
}

function selectJsPlumbViewBy(transitionId, selectedRow, refStructureId, dataViewId) {
    selectedTransitionId = transitionId;
    $.ajax({
        type: 'post',
        url: 'mdprocessflow/getInteractiveTransitionNewListData',
        dataType: 'json',
        data: {
        ...
        success: function (data) {
            if (data.status === 'success') {
                ...
                statusAllConnections(data, refStructureId, dataViewId);
...
function statusAllConnections(data, refStructureId, dataViewId) {
    ...
    if (data['object'].length != 0) {
        ...
        $.each(data['object'], function (index, value) {
            wfmHtml += setIcon(value, refStructureId, dataViewId);
        });
