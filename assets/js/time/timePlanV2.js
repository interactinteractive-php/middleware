...
$(function () {
    ...
    $('body').on("change", "select[name='calcId']", function () {
        var $this = $(this);
    ...
    $('body').on('change', '#departmentId', function() {
        $.ajax({
            type: 'post',
            url: 'mdtimestable/getPositionList',
            dataType: "json",
            data: {"param": $(this).val()},
            beforeSend: function () {
                Core.blockUI({
    ...
    $("body").on("click", tnaTimeEmployeePlanWindowId + " .balanceReload", function () {
        var timePlanUniqId = $(this).attr('data-view-id');
        
        getEmployeePlanList(timePlanUniqId);
        if ($("input[name='newDepartmentId[]']", tnaTimeEmployeePlanWindowId).val() !== "") {
    ...
    $("body").on("keydown", tnaTimeEmployeePlanWindowId + " input[name=\"stringValue\"]", function (e) {
        var keyCode = (e.keyCode ? e.keyCode : e.which);
        
        if (keyCode == 13) {
            $(".search-tms-plan-btn", tnaTimeEmployeePlanWindowId).click();
            e.preventDefault();
            return false;        
        }
    });
    
    $("body").on('change', tnaTimeEmployeePlanWindowId + ' #isWorkingDays', function() {
        var _this = $(this);
        ...
        if (_this.is(':checked')) {
            ...
            tbl.find('th.weekday').each(function(){
                var monId = $(this).data('month');
                tbl.find('.month-'+monId).attr('colspan', Number(tbl.find('.month-'+monId).attr('colspan')) - 1);
            });
    ...
    $("body").on('change', tnaTimeEmployeePlanWindowId + ' #employeeCurrentMonthTime', function() {
        var thisval = $(this).val();
    ...
    $("body").on('change', tnaTimeEmployeePlanWindowId + ' #isPositionWorkingDays', function() {
        PNotify.removeAll();
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceExportExcel', function() {
        $.blockUI(".block-ui");
        $.download('mdtimestable/timePlanExport/1', 'form', $("#tnaTimeEmployeePlanForm").serialize(), 'post', '');
        $.unblockUI(".block-ui");
    });
    
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balancePlanCountExportExcel', function() {
        $.blockUI(".block-ui");
        $.download('mdtimestable/timePlanExport/2', 'form', $("#tnaTimeEmployeePlanForm").serialize(), 'post', '');
        $.unblockUI(".block-ui");
    });
    
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balancePaste', function() {
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceVerifyItem', function() {
        ...
        if(userSessionIsFull()) {
            ...
            if (wfmStatusCode == 'confirmedByExecutive') {
                ...
                $.ajax({
                    type: 'post',
                    url: 'mdtimestable/confirmLastDate',
                    dataType: "json",
                    beforeSend: function () {
                        Core.blockUI({
                            animate: true
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceVerify', function() {
        var _this = $(this);
        var wfmStatusCode = _this.attr('data-status-code');
        var planYear   = $('#planYear option:selected').val();
        var planMonth  = $('#planMonth option:selected').val();
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceOk', function() {
        ...
        dialogname.dialog({
          ...
          buttons: [
            { text: plang.get('yes_btn'), click: function() {
                $.ajax({
                  type: 'post',
                  url: 'mdtimestable/okEmployeePlanMonth',
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceNo', function() {
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceChoseDelete', function() {
        ...
        if (checkSelectedCell()) {
            if(userSessionIsFull()) {
                ...
                if ($('select[name="calcId"]', tnaTimeEmployeePlanWindowId).length) {
                    _selectedPlanMonth = $('select[name="calcId"] option:selected', tnaTimeEmployeePlanWindowId).attr('startdate') + ' - ' + $('select[name="calcId"] option:selected', tnaTimeEmployeePlanWindowId).attr('enddate');
                }
                $("#" + $dialogName).empty().html('Төлөвлөгөөг устгахдаа итгэлтэй байна уу?');
                $("#" + $dialogName).dialog({
                    cache: false,
                    resizable: false,
                    bgiframe: true,
                    ...
                    buttons: [
                        {text: 'Тийм', class: 'btn green-meadow btn-sm', click: function () {
                            ...
                            $.ajax({
                                type: 'post',
                                url: 'mdtimestable/deleteEmployeePlanBalance',
                                ...
                                Core.unblockUI();
                                var $dialogName = 'dialog-confirm-delete-balance';
                                if (!$("#" + $dialogName).length) {
                                    $('<div id="' + $dialogName + '"></div>').appendTo('body');
                                }
                                
                                if(data.status === 'locker') {
                                    var dString = 'Дараах ажилтнуудын цагийн мэдээлэл <strong>түгжигдсэн</strong> байна. <br>Энэ төлөвлөгөөг устгах боломжгүй байна.';
                                    dString += '<ol class="mt10">';
                                    for(var s = 0; s < data.data.length; s++) {
                                        dString += '<li>' + data.data[s] + '</li>';
                                    }
                                    dString += '</ol>';
                                    
                                    $("#" + $dialogName).empty().html(dString);
                                    $("#" + $dialogName).dialog({
                                        cache: false,
                                        resizable: false,
                                        bgiframe: true,
                                ...
                                var dString = 'Дараах ажилтнуудын цагийн мэдээлэл <strong>татагдсан</strong> байна. <br><strong style="color:red">Цагийн мэдээлэлээ устгаад</strong> энэ үйлдлээ үргэлжлүүлэх үү?';
                                dString += '<ol class="mt10">';
                                for(var s = 0; s < data.data.length; s++) {
                                    dString += '<li>' + data.data[s] + '</li>';
                                }
                                ...
                                $("#" + $dialogName).dialog({
                                    ...
                                    buttons: [
                                        {text: 'Тийм', class: 'btn green-meadow btn-sm', click: function () {                                    
                                            $.ajax({
                                                type: 'post',
                                                url: 'mdtimestable/deleteEmployeePlanV2',
                                                data: $("#tnaBalancePlanGridForm").serialize() + $("#tnaTimeEmployeePlanForm").serialize() + '&userWfmStatusCode=' + wfmStatusCode + '&userWfmStatusId=' + wfmStatusId,
                                                dataType: "json",
                                                beforeSend: function () {
                                                    Core.blockUI({
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .balanceDelete', function() {
    ...
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' .stoggler', function () {
        ...
        if (_rightsidebarstatus === "closed") {
          _centersidebar.removeClass("col-md-12").addClass("col-md-10");
          _rightsidebar.addClass("col-md-2 pr0");
          ...
          _rightsidebar.find(".right-sidebar-content").hide(
                  "slide", {direction: "right"}, 1200,
                  function () {
                      _centersidebar.removeClass("col-md-10").addClass("col-md-12");
                      _rightsidebar.removeClass("col-md-2");
    ...
    $("body").on("mouseover", tnaTimeEmployeePlanWindowId + ' .stoggler',  function () {
        $(this).css({
    ...
    $("body").on("mouseleave", tnaTimeEmployeePlanWindowId + ' .stoggler', function () {
        $(this).css({
...
function verifyPlanDate(planYear, planMonth, departmentId, approveLastDate, wfmStatusId, wfmStatusCode, confirmDescription) {
    $.ajax({
        type: 'post',
        url: 'mdtimestable/verifyEmployeePlanMonth',
...
function verifyDayPlanDate(confirmDescription) {
    ...
    $.ajax({
        type: 'post',
        url: 'mdtimestable/verifyEmployeePlanDay',
        ...
        buttons: [
            {text: 'Тйим', class: 'btn green-meadow btn-sm', click: function () {
                ...
                if ($("#tnaTimeEmployeePlanForm").valid()) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: 'mdtimestable/deleteEmployeePlanV2Month',
                        data: $("#tnaBalancePlanGridForm").serialize() + $("#tnaTimeEmployeePlanForm").serialize() + '&userWfmStatusCode=' + wfmStatusCode + '&userWfmStatusId=' + wfmStatusId,
                        dataType: "json",
                        beforeSend: function () {
                            Core.blockUI({
...
function copyPasteBalanceDate(planYear, planMonth, cPlanYear, cPlanMonth, cDepartmentId) {
    $.ajax({
            type: 'post',
            url: 'mdtimestable/saveEmployeePlanMonth',
            ...
            success: function (data) {
                ...
                $.ajax({
                    type: 'post',
                    url: 'mdtimestable/employeePlanListMainDataGrid',
                    data: { 
...
function getEmployeePlanList(timePlanUniqId) {
    if ($("input[name='newDepartmentId[]']", tnaTimeEmployeePlanWindowId).val() === "" && ($("select[name='groupId[]']", tnaTimeEmployeePlanWindowId).val() == null || !$("select[name='groupId[]']", tnaTimeEmployeePlanWindowId).val().length)) {
        PNotify.removeAll();
        new PNotify({
            title: 'Анхааруулга',
            text: 'Салбар нэгжээ сонгоно уу!',
            type: 'warning',
            sticker: false
        });        
        return;
    ...
    setTimeout(function(){
        $.ajax({
            type: 'post',
            url: 'mdtimestable/existMetaIdPlan',
            dataType: "json",
            async: false,
            success: function (data) {
                if (Object.keys(data).length) {    
                    $("#listfromdv", tnaTimeEmployeePlanWindowId).val("1");
                    $.ajax({
                        type: 'post',
                        url: 'mdtimestable/employeePlanListMainDataGridNewV4',
                        data: {"params": $("#tnaTimeEmployeePlanForm", tnaTimeEmployeePlanWindowId).serialize()},
                        dataType: "json",
                        beforeSend: function () {
                        },
                        success: function (data) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: 'mdtimestable/employeePlanListMainDataGridNewV3',
                        data: {"params": $("#tnaTimeEmployeePlanForm", tnaTimeEmployeePlanWindowId).serialize()},
                        dataType: "json",
                        beforeSend: function () {
                        },
...
function getEmployeePositionPlanList(isInsert) {
    $.ajax({
        type: 'post',
        url: 'mdtimestable/employeePositionPlanListMainDataGrid',
...
function deleteEmployeePositionPlanList() {
    ...
    if ($("#tnaTimeEmployeePlanForm").valid()) {
        $.ajax({
            type: 'post',
            url: 'mdtimestable/deleteEmployeePositionPlanListMainDataGrid',
            data: { 
                "params": $("#tnaTimeEmployeePlanForm").serialize()
            },
            dataType: "json",
...
function reloadEmployeePlanList(data) {
    if (data != undefined) {
        $("#tnaBalanceGrid", tnaTimeEmployeePlanWindowId).html(data).promise().done(function() {
            ...
            pagerElement.find('.pf-custom-pager-total > span:not(".pf-custom-pager-total-selectedday")').text(totalRowNumber);
        ...
        if ($("#onlyWorkingDay").val() == '1') {
            ...
            // }
            // _tblColSpan.attr('colspan', parseInt($bcolspan) - parseInt($colspan));
            $('input[name="isWorkingDays"]').prop('checked', true).trigger('change');
            $('input[name="isWorkingDays"]').parent().addClass('checked');            
        }
        ...
        $("#tnaBalanceGrid table tbody").selectable({
            ...
            stop : function (event, ui) {
                ...
                var planId, departmentId = sumTime = 0;
                var row = _cells.closest('tr');
                $.each(_cells, function(){
                    sumTime += Number($(this).text());
                });

                $('.pf-custom-pager-total-selectedday').html('Сонгосон <strong>'+_cells.length+'</strong> өдөр <strong style="color:red">'+(isNaN(sumTime) ? '' : sumTime) +'</strong> цаг.');
                if (_cells.length === 1) {
...
function changeEmployeePlanTime() {
    $("body").on('click', tnaTimeEmployeePlanWindowId + ' input:radio[name="plan"]', function(){
        var $this = $(this);
        ...
        $.ajax({
            ...
            success: function (data) {         
               ...
               var $dialogName = 'dialog-confirm-change-plant';
                if (!$("#" + $dialogName).length) {
                    $('<div id="' + $dialogName + '"></div>').appendTo('body');
                }
               
                var _selectedPlanMonth = $('#planMonth option:selected', tnaTimeEmployeePlanWindowId).text();
                $("#" + $dialogName).empty().html('<strong>' + $this.closest('label').text() + '</strong> ' + plang.get('change-plan-warningmsg'));
                $("#" + $dialogName).dialog({
                    cache: false,
                    resizable: false,
                    bgiframe: true,
                    ...
                    buttons: [
                        {text: 'Тийм', class: 'btn green-meadow btn-sm', click: function () {                            
                            $.ajax({
                                type: 'post',
                                url: 'mdtimestable/saveEmployeePlanV2',
                                data: $("#tnaBalancePlanGridForm").serialize() + $("#tnaTimeEmployeePlanForm").serialize(),
                                dataType: "json",
                                beforeSend: function () {
                                    Core.blockUI({
...
function getEmptySideBar (timePlanUniqId) {
    $.ajax({
        type: 'post',
        url: 'mdtimestable/getEmptySideBarV2',
        dataType: "json",
        data: {
            "year": $("#planYear", '.tnaTimeEmployeePlan-'+ timePlanUniqId).val(), 
            "month": $("#planMonth", '.tnaTimeEmployeePlan-'+ timePlanUniqId).val(), 
            "departmentId": $("#departmentId", '.tnaTimeEmployeePlan-'+ timePlanUniqId).val(), 
            'uniqId': timePlanUniqId,
            "params": $("#tnaTimeEmployeePlanForm", tnaTimeEmployeePlanWindowId).serialize()
        },
        async: false,
        success: function (data) {
...
function addUserSessionCount() {
    $.ajax({
        type: 'post',
        url: 'mdtimestable/addUserSessionCount',
        dataType: "json",
        async: false,
        success: function (data) {
            result = data;
...
function initTimePlanList(rowData, timePlanUniqId, cell) {
    ...
    setTimeout(function(){
        $.ajax({
            type: 'post',
            url: 'mdtimestable/employeePlanListV2',
            dataType: "json",
            async: false,
            data: {
                "params": $("#tnaTimeEmployeePlanForm", '.tnaTimeEmployeePlan-'+ timePlanUniqId).serialize(),
                "requestType": type,
                "year": $("#planYear", '.tnaTimeEmployeePlan-'+ timePlanUniqId).val(), 
                "month": $("#planMonth", '.tnaTimeEmployeePlan-'+ timePlanUniqId).val(),
                "uniqId": timePlanUniqId
            ...
            success: function (data) {
                ...
                for(var i = 0; i < employeePlanCount; i++) {
                    ...
                    if (jstmsCustomerCode === 'gov') {
                        html += '<td style="min-width:40px !important"><a href="javascript:;" onclick="dataViewTimePlanEmployeeGrid()">' + row.PLAN_COUNT + '</a></td>';
                    }
                    html += '</tr>';
                }
                
                $("body").find(tnaTimeEmployeePlanWindowId + " .defaultShowEmployeePlanTime").closest('.slimScrollDiv').remove();
                $("body").find(tnaTimeEmployeePlanWindowId + " .defaultShowEmployeePlanTime").remove();
                $("body").find(tnaTimeEmployeePlanWindowId + " .plan-seperator").removeClass('d-none');
                sidebarContent.html('<div class="row">'
                                        + '<div class="col-md-12">'
                                            + '<input type="text" id="tnatimeplan" class="form-control form-control-sm stringInit mb5" placeholder="Хайх" value="">'
                                        + '</div>'
                                    + '</div>'
                                    + '<div class="panel panel-default bg-inverse timePlanScroller" style="max-height: 320px;">'
                                        + '<table class="table sheetTable timePlanList" id="tna_timeplan_list">'
                                            + '<thead class="hidden">'
                                                + '<tr>'
                                                    + '<th>Нэр</th>'
                                                    + '<th>Цаг</th>'
                                                    + (jstmsCustomerCode === 'gov' ? '<th style="min-width:40px !important"></th>' : '')
                                                + '</tr>'
                                            + '</thead>'
            ...
            if ($("#onlyWorkingDay").val() == '1') {
                $('input[name="isWorkingDays"]').prop('checked', true);
                $('input[name="isWorkingDays"]').parent().addClass('checked');
            }            
                ...
                buttons: [
                    {text: data.save_btn, class: 'btn btn-sm green-meadow', click: function () {
                        ...
                        if ($("#archivForm", dialogName).valid()) {
                            ...
                            var result = sendArchivAjax($("#description").val(), tnaPlanUniqId);
...
function sendArchivAjax(param, tnaPlanUniqId) {
    ...
    $.ajax({
        type: 'post',
        url: 'mdtimestable/sendArchivV2',
        data: {"params":$("#tnaTimeEmployeePlanForm", tnaTimeEmployeePlanWindowId).serialize(), "description": param},
        dataType: "json",
        async: false,
        beforeSend: function () {
            Core.blockUI({
...
function getViewArchiv(elem, uniqId) {
    ...
    var departmentIds = '';
    $('input[data-path="newDepartmentId"]').each(function(){
    ...
    if(archivId.length > 0 && departmentIds != null) {
        $.ajax({
            type: 'post',
            url: 'mdtimestable/getArchivPlanListMainDataGridV2',
            data: {"planLogId": archivId, "departmentId": departmentIds, "isChild":$("#tnaTimeEmployeePlanForm", tnaTimeEmployeePlanWindowId).find('input[name="isChild"]').is(':checked') ? 1 : 0},
            dataType: "json",
            beforeSend: function () {
                Core.blockUI({
                    animate: true
...
function recoveryArchivDailog(elem) {
    ...
    if (archivId.length > 0 && departmentIds != null) {
        ...
        $.ajax({
            type: 'post',
            url: 'mdtimestable/recoveryArchivDialog',
...
function checkPlanCurrentMonthDay() {
    ...
    $.ajax({
        type: 'post',
        url: 'mdtimestable/checkPlanCurrentMonthDay',
...
function recoveryArchivData(param) {
    ...
    $.ajax({
        type: 'post',
        url: 'mdtimestable/recoveryArchivDataV2',
        data: param,
        dataType: "json",
        async: false,
        beforeSend: function () {
...
function setArchivComboData(param) {
    $.ajax({
        type: 'post',
        url: 'mdtimestable/getArchivList',
        data: param,
        dataType: "json",
        async: false,
        beforeSend: function () {
